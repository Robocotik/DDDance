...
const MixamoViewer: React.FC<MixamoViewerProps> = ({ result }) => {
	...
	const getSegmentUrlRef = useRef<((idx: number) => string | null) | null>(
		null,
	);
	...
	useEffect(() => {
		loadSegmentAnimationRef.current = loadSegmentAnimation;
	}, [loadSegmentAnimation]);
	useEffect(() => {
		getSegmentUrlRef.current = getSegmentUrl;
	}, [getSegmentUrl]);
	...
	const totalDuration = segments.reduce(
		(acc, s) => acc + getSegmentDuration(s),
		0,
	);
				...
				<div style={{ width: '50%', maxWidth: 900 }}>
					...
					>
						...
						<button
							...
							style={{
								padding: '6px 12px',
								cursor: result?.full_glb_key ? 'pointer' : 'not-allowed',
								background: '#4a6fa5',
								color: 'white',
								border: 'none',
								borderRadius: 4,
									...
									return (
										<button
											...
											style={{
												...
												alignItems: 'center',
												fontSize: 13,
												opacity: hasAnim ? 1 : 0.5,
											}}
										>
											<span style={{ fontWeight: isActive ? 600 : 400 }}>
												{seg.index + 1}.{' '}
												{seg.label ?? `Сегмент ${seg.index + 1}`}
												{!hasAnim && ' (нет)'}
											</span>
