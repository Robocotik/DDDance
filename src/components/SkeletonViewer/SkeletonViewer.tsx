...
const SkeletonViewer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getConnectionColor = (from: number, to: number): number => {
      if ((from === 11 || to === 11) && (from === 12 || to === 12)) return 0x44aaff;
    ...
    const convertJoints = (frame: Frame): THREE.Vector3[] => {
      ...
      const scale = 1.5;
      return frame.joints.map(joint => new THREE.Vector3(
    ...
    const smoothJoints = (raw: THREE.Vector3[], prev: THREE.Vector3[] | null, alpha: number): THREE.Vector3[] => {
      if (!prev || prev.length !== raw.length) return raw.map(p => p.clone());
      return raw.map((p, i) => new THREE.Vector3(
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setStatus(`Готово: ${totalFramesCount} кадров, ${Math.round(fps)} FPS${data.segments?.length ? `, ${data.segments.length} сегментов` : ''}`);
      } catch (err) {
    ...
    <section className={styles.section}>
      ...
      <div className={styles.controls}>
        <div className={styles.fileButtons}>
          {['skeleton.json', 'b_result.json', 'b_result_2.json', '000.json'].map(f => (
            <button
              key={f}
        ...
        <div className={styles.playbackControls}>
          ...
          <div className={styles.sliderGroup}>
            <span>Кадр: {currentFrame} / {totalFrames}</span>
            <input
              type="range"
              min={0}
              max={totalFrames - 1}
